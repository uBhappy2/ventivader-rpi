...
class TxCharacteristic(Characteristic):
    # Format is (pressure, volume, flow)
    values = [(1,2,3), (3,2,1), (2,3,1), (2,1,2)]
    index = 0
    ...
    def send_json_data(self):
        #print("will send a new value")
        self.send_tx("{\"pressure\":%s,\"volume\":%s,\"flow\":%s}" % self.values[self.index])
        self.index = self.index + 1
        self.index = self.index % len(self.values)
        timer = threading.Timer(1.0, self.send_json_data)
        timer.start()
